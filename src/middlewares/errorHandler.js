...
export function errorHandler(err, req, res, next) {
  ...
  if (err.type === "entity.parse.failed") {
    return respuesta.error(req, res, "JSON inválido en el body", 400);
  }
  ...
  if (
    ...
    return respuesta.error(req, res, err.message, status);
  }

  // Errores de Prisma
  const prismaErr = handlePrismaError(err);
  if (prismaErr) {
    return respuesta.error(req, res, prismaErr.message, prismaErr.statusCode);
  }
